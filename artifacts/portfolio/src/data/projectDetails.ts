...
export const PROJECT_DETAILS: Record<number, ProjectDetail> = {
  ...
  8: {
    ...
    duration: "7 weeks",
    longDescription:
      "PakBizBranches Directory Website is a web platform that lets businesses present professional directory listings with fast performance and clear organization. Users browse business profiles and locations with server-side rendered pages for high speed, backed by Next.js, Prisma, PostgreSQL, and TypeScript. It powers the live directory site pakbizbranhces.online.",
    features: [
  ...
  20: {
    ...
    duration: "4 weeks",
    longDescription:
      "AI SEO Website Audit & Growth Assistant is an AI-powered Progressive Web App (PWA) designed to help website owners, developers, freelancers, agencies, and digital marketers analyze and improve the overall health and growth potential of a website. Instead of requiring multiple separate tools for SEO, performance, accessibility, security, and optimization recommendations, the application brings these capabilities together into a single platform.\n\nThe application starts with a real-time website audit engine. A user provides a website URL, and the system analyzes the target website against several important technical and SEO factors. The audit examines metrics such as Largest Contentful Paint (LCP), Time to First Byte (TTFB), and overall performance, while also evaluating SEO elements including meta titles, meta descriptions, canonical tags, Open Graph metadata, and heading hierarchy from H1 through H6. The application also checks important accessibility and security factors, including HTTPS usage, image alternative text, mobile viewport configuration, and other website health indicators.\n\nAfter the audit is completed, the results are presented through an interactive analytics dashboard. The dashboard converts the collected information into an overall Growth Score from 0 to 100 and separates the results into categories such as Performance, SEO, Security, and Accessibility. Instead of simply displaying technical numbers, the application identifies priority areas so users can understand which improvements can have the greatest impact. Users can also export the audit information as a structured JSON report for further analysis or record keeping.\n\nOne of the main features of the platform is its AI Neural Growth Assistant. The AI is connected through a custom Next.js API route and uses Google's Gemini models. What makes the assistant different from a basic chatbot is that it can receive the actual audit results of the user's website as context. For example, if a website has a slow LCP or high TTFB, the AI can use those real metrics when generating recommendations. It can suggest performance improvements such as image optimization, WebP conversion, CDN configuration, and script optimization. It can also generate SEO content such as optimized H1 headlines and meta descriptions based on the website's context.\n\nThe application also includes an Audit History system. Previous audit results can be stored locally and optionally synchronized with Firebase Firestore. This allows users to review previous reports, search and manage historical audits, and compare website scores over time. This makes the application useful not only for one-time analysis but also for continuously monitoring website improvements.\n\nAnother important part of the project is its Progressive Web App architecture. The application includes a web manifest and custom service worker, allowing it to be installed on devices such as Android, iOS, Windows, and macOS. Cached static assets also provide offline capabilities. The interface is responsive and uses Tailwind CSS with modern UI elements, animations, and light/dark visual modes.\n\nFor authentication, the platform uses Firebase Authentication, supporting email/password accounts while also providing a guest mode so users can begin auditing websites without creating an account. The core application is built with Next.js 14 and React 18, while Gemini provides the AI capabilities, Firebase handles authentication and cloud storage, and Vercel provides the production deployment environment.\n\nOverall, the application works as a complete workflow: enter a website URL → perform a technical and SEO audit → calculate growth and category scores → display prioritized recommendations → provide AI-powered, website-specific optimization guidance → save and compare historical audits. This combination of automated auditing, AI recommendations, analytics, authentication, cloud storage, and PWA functionality makes it more than a simple SEO checker; it is a complete website growth and optimization assistant.",
    features: [
      "Real-time technical website audit engine analyzing LCP, TTFB, meta tags, Open Graph, and H1-H6 hierarchy",
      "Interactive analytics dashboard with Growth Score (0-100) across SEO, Performance, Security & Accessibility",
      "AI Neural Growth Assistant powered by Google Gemini models and custom Next.js API routes",
      "Contextual AI recommendations utilizing actual website performance and audit metrics",
      "Audit History system with local storage & optional Firebase Firestore synchronization",
      "Installable Progressive Web App (PWA) architecture supporting Android, iOS, macOS, and Windows",
      "Firebase Authentication supporting Email/Password accounts and instant Guest Mode",
      "Structured JSON audit report exporting for archival, analysis, and client delivery",
      "Responsive modern UI with light/dark theme modes using Tailwind CSS and React 18",
    ],
    challenges:
      "Connecting Google's Gemini models with real-time audit metric contexts (LCP, TTFB, meta tag analysis) through Next.js API routes while ensuring responsive offline PWA caching and seamless Firebase sync required careful API contract design, asynchronous event ordering, and robust error handling.",
  },
};
